...
static int portable_extract_by_path(
        ...
        if (r == -EISDIR) {
                ...
                r = dissect_image(d->fd, NULL, NULL, d->diskseq, DISSECT_IMAGE_READ_ONLY|DISSECT_IMAGE_REQUIRE_ROOT|DISSECT_IMAGE_DISCARD_ON_LOOP|DISSECT_IMAGE_RELAX_VAR_CHECK, &m);
                if (r == -ENOPKG)
...
static int make_marker_text(const char *image_path, OrderedHashmap *extension_images, char **ret_text) {
        ...
        ORDERED_HASHMAP_FOREACH(ext, extension_images) {
                ...
                if (!strextend(&text, ":", escaped, NULL))
                        return -ENOMEM;
        }
