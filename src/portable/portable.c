...
static int install_chroot_dropin(
        ...
        text = strjoin(PORTABLE_DROPIN_MARKER_BEGIN, m->image ?: image_path, PORTABLE_DROPIN_MARKER_END "\n");
        if (!text)
                return -ENOMEM;

        if (endswith(m->name, ".service")) {
                const char *root_type = IN_SET(type, IMAGE_DIRECTORY, IMAGE_SUBVOLUME) ? "RootDirectory=" : "RootImage=";
                ...
                if (m->image && !streq(m->image, image_path))
                        if (!strextend(&text, "MountImages=", m->image, "\n", NULL))
                                return -ENOMEM;
        }
