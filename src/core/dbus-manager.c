...
static int method_enqueue_units_jobs(sd_bus_message *message, void *userdata, sd_bus_error *reterr_error) {
         ...
         * actual reload-vs-restart choice is unit-specific and applied per-unit below. */
        ...
        STRV_FOREACH(name, names) {
                ...
                /* Apply per-unit validation/mangling. The mangled type is unit-specific (e.g. with
                 * reload_if_possible, units that support reload get JOB_RELOAD_OR_START while others
                 * keep JOB_RESTART), so we record the per-unit type in the hashmap. */
                r = unit_queue_job_check_and_mangle_type(u, &t, reload_if_possible, reterr_error);
                if (r < 0)
                        return r;

                if (hashmap_put(units_with_types, u, INT_TO_PTR(t + 1)) < 0)
                        return -ENOMEM;
        }
        ...
        r = manager_add_jobs_full(m, units_with_types, mode, /* extra_flags= */ 0, /* affected_jobs= */ NULL, reterr_error, jobs);
        if (r < 0)
                return r;
