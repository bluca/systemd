...
static int luo_try_restore_fd(Manager *m, const char *cgroup_path, const char *fdname, int fd) {
        ...
        log_info("Restored LUO fd '%s' for unit '%s'.", fdname, u->id);
...
int manager_luo_restore_fd_stores(Manager *m) {
        ...
        log_info("Found LUO session '%s', restoring fd stores.", LUO_SESSION_NAME);
        ...
        JSON_VARIANT_OBJECT_FOREACH(cgroup_path, fds_json, mapping) {
                ...
                JSON_VARIANT_ARRAY_FOREACH(entry, fds_json) {
                        ...
                        if (r == -ENOENT) {
                                ...
                                log_info("LUO: holding fd '%s' for cgroup '%s' until unit is loaded.", fdname, cgroup_path);
        ...
        if (r < 0)
                log_warning_errno(r, "Failed to finish LUO session '%s': %m", LUO_SESSION_NAME);

        if (n_total > 0)
                log_info("Restored %d fd(s) total from LUO session.", n_total);

        if (!hashmap_isempty(m->luo_held_fds))
                log_info("Holding LUO fd(s) for units not yet loaded.");
...
int manager_luo_process_held_fds(Manager *m) {
        ...
        log_info("Processing pending LUO held fd(s)...");
        ...
        if (n_restored > 0)
                log_info("Restored %d held LUO fd(s).", n_restored);
...
int manager_luo_try_restore_held_fds_for_unit(Unit *u) {
        ...
        if (n_restored > 0)
                log_unit_info(u, "Restored %d held LUO fd(s).", n_restored);
...
int manager_luo_serialize_fd_stores(Manager *m, FILE **ret_f, FDSet **ret_fds) {
        ...
        if (n_serialized == 0) {
                log_info("No fd store entries to serialize for LUO.");
                *ret_f = NULL;
                *ret_fds = NULL;
                return 0;
        }
