...
static int do_security(int argc, char *argv[], void *userdata) {
        ...
        if (arg_security_policy) {
                ...
                r = search_and_fopen_nulstr("systemd-analyze-security.policy", "re", /*root=*/ NULL, CONF_PATHS_NULSTR("systemd"), &f);
                if (r < 0 && r != -ENOENT)
                        return r;

                if (f != NULL) {
                        r = json_parse_file(f, NULL, /*flags=*/ 0, &policy, &line, &column);
                        if (r < 0)
                                return log_error_errno(r, "[%u:%u] Failed to parse JSON policy: %m", line, column);
                }
        }
...
static int parse_argv(int argc, char *argv[]) {
        ...
        while ((c = getopt_long(argc, argv, "hH:M:", options, NULL)) >= 0)
                ...
                case ARG_OFFLINE:
                        if (optarg) {
                                ...
                                if (r < 0)
                                        return log_error_errno(SYNTHETIC_ERRNO(EINVAL),
                                                               "Failed to parse --offline= argument.");
                ...
                case ARG_SECURITY_POLICY:
                        r = parse_path_argument_and_warn(optarg, /* suppress_root= */ false, &arg_security_policy);
                        if (r < 0)
                                return r;
                        break;
        ...
        if (arg_json_format_flags != 0 && !streq_ptr(argv[optind], "security"))
                return log_error_errno(SYNTHETIC_ERRNO(EINVAL),
