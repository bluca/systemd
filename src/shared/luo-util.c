...
/* Kernel API defined at https://docs.kernel.org/next/userspace-api/liveupdate.html
 * The /dev/liveupdate is a single-owner singleton, only a single process at any
 * given time can open it. Callers can create named "sessions", and then add FDs to
 * them. The session name can be used to retrieve the session after reboot. To
 * identify an FD, a 64bit token is passed in, and the caller is responsible for
 * coming up with the token and tracking them. */

int luo_open_device(void) {
        _cleanup_close_ int fd = -EBADF;

        fd = open("/dev/liveupdate", O_RDWR|O_CLOEXEC);
        if (fd < 0)
                return -errno;
...
int luo_create_session(int device_fd, const char *name) {
        ...
        strncpy((char *) args.name, name, sizeof(args.name));
...
int luo_parse_serialization(sd_json_variant **ret, int **ret_fds, size_t *ret_n_fds) {
        ...
        const char *luo_fd_str = getenv("LUO_SERIALIZE_FD");
        if (!luo_fd_str) {
                *ret = NULL;
                *ret_fds = NULL;
                *ret_n_fds = 0;
        ...
        if (r < 0 || serialize_fd < 0)
                return log_warning_errno(r < 0 ? r : SYNTHETIC_ERRNO(EBADF),
                                         "Failed to parse LUO_SERIALIZE_FD='%s', ignoring: %m", luo_fd_str);
        ...
        if (r < 0)
                return log_warning_errno(r, "Failed to read LUO serialization: %m");

        r = sd_json_parse(text, SD_JSON_PARSE_MUST_BE_OBJECT, &root, /* reterr_line= */ NULL, /* reterr_column= */ NULL);
        if (r < 0)
...
int luo_preserve_fd_stores(sd_json_variant *serialization, int *ret_session_fd) {
        ...
        JSON_VARIANT_OBJECT_FOREACH(cgroup_path, entries, serialization) {
                ...
                JSON_VARIANT_ARRAY_FOREACH(entry, entries) {
                        ...
                        if (!type_json || !name_json || !fd_json)
                                continue;
                        ...
                        if (streq(type, "fd")) {
                                ...
                                if (r < 0) {
                                        log_debug_errno(r, "Failed to preserve LUO fd %i with fd_index %" PRIu64 ": %m", fd, fd_index);
                                        continue;
                                }
        ...
        mapping_fd = memfd_new_and_seal("luo-mapping", mapping_text, strlen(mapping_text));
        if (mapping_fd < 0)
...
int fd_is_luo_session(int fd) {
        ...
        return startswith(path, "anon_inode:[luo_session]") != NULL;
}
...
int fd_get_luo_session_name(int fd, char **ret) {
        ...
        if (!name)
                return -EMEDIUMTYPE; /* not a LUO session fd */
