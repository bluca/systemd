...
int table_print_with_pager(
        ...
        if (!json_format_flags || json_format_flags & (JSON_FORMAT_PRETTY|JSON_FORMAT_PRETTY_AUTO))
                (void) pager_open(pager_flags);

        saved_header = t->header;
        t->header = show_header;
        if (json_format_flags)
...
int table_set_json_field_name(Table *t, size_t column, const char *name) {
        ...
        if (name) {
                ...
                if (!GREEDY_REALLOC0(t->json_fields, t->n_json_fields, m))
                        return -ENOMEM;
