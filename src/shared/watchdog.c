...
static int get_pretimeout_governor(char **ret_gov) {
        ...
        r = read_full_virtual_file(sys_fn, ret_gov, NULL);
        if (r < 0)
                return r;
...
static int set_pretimeout_governor(const char *governor) {
        ...
        r = write_string_file(sys_fn,
                              governor,
                              WRITE_STRING_FILE_DISABLE_BUFFER | WRITE_STRING_FILE_VERIFY_ON_FAILURE);
        if (r < 0)
...
static int watchdog_set_pretimeout(void) {
        ...
        if (ioctl(watchdog_fd, WDIOC_SETPRETIMEOUT, &sec) < 0) {
                ...
                return log_error_errno(errno, "Failed to set pretimeout to %s: %m", format_timespan(buf, sizeof(buf), sec, USEC_PER_SEC));
        }
